	...
	def Get_distance_callback(self,topic):
		...
		if self.distanceA < self.min_distance_LR or self.distanceB < self.min_distance or self.distanceC < self.min_distance or self.distanceD < self.min_distance or self.distanceE < self.min_distance or self.distanceF < self.min_distance_LR:
			if self.nav_mode_flag == 1:
				# 遇到障碍物取消导航
				if self.nav_cancel_flag == 0:
					self.nav_cancel_flag = 1
					print(GREEN+"distance too close,cancel nav"+RESET)
				self.Pub_NavGoal_Cancel()
				my_cmd_vel.linear.x = 0.2
				self.Cmd_vel_pub.publish(my_cmd_vel)
			...
			if self.avoid_flag == 1:
				...
				if (rospy.Time.now() - self.begin_time).secs>=1:
					...
					if self.nav_mode_flag ==1:
						self.drive_line = 1
						self.last_drive_line = self.drive_line
			...
			# 避障完成，重新发布目标点
			if self.last_drive_line == 1 and self.drive_line == 0:
				self.last_drive_line=0
				print(GREEN+"avoid the obstacle , start nav"+RESET)
				self.nav_cancel_flag = 0
				self.Pub_Nav_Position()
