...
import argparse
from TACO import make_agent
from Replay_Buffer_taco import ReplayBuffer
...
def evaluate(eval_episodes=0,sample_stochastically=False):
    all_ep_rewards = []
    step_count = []
    collison = 0
    success = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--eval_freq", default=1e4, type=int, help="After how many steps to perform the evaluation 5e3")
    ...
    io = IOStream('./results' + '/voxelnet_laser.log')
    ...
    while timestep < args.max_timesteps:  # 5000000
        ...
        if timestep < args.init_steps:
            ...
            with eval_mode(agent):
                action = agent.sample_action(goal, pcd)
                unnorm_action = np.array([
        ...
        next_goal, next_pcd, reward, done, target = env.step(unnorm_action)
        ...
        if np.any(pcd) and np.any(next_pcd):
            replay_buffer.add(goal, pcd, action, reward, next_goal, next_pcd, done)
